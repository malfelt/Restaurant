...
public class Table extends Thread {
    ...
    public void serve(List<Product> menu){
        ...
        try {
            ...
            if (eating) {
                terminal.print(prefix + "\nЯ сажусь есть. Мне есть " + product.getTime() + " секунд.");
                Thread.sleep(product.getTime());
                product.setCount(product.getCount() - 1);
                terminal.print(prefix + "\nЯ ухожу,потому что съел блюдо!");
                person = Restaurant.getPersonFromQueue();
                if(person == null) {
                    terminal.print(prefix + "\nОчередь в ресторане кончилась. Скоро закрываемся");
                    return;
                }
            } else {
                if (!checkTime) {
                    terminal.print(prefix + "\nЯ не буду это заказывать, потому что готовят медленно.");
                } else if(!checkPreferences) {
                    terminal.print(prefix + "\nЯ не буду это заказывать, потому что мне не подходит");
                } else {
                    terminal.print(prefix + "\nЯ не буду это заказывать, потому что мне не хватает средств");
                }
                if (person.getAttempt() == 3) {
                    terminal.print(prefix + "\nЯ ухожу, потому что 3 попытки закончились!");
                    person = Restaurant.getPersonFromQueue();
                    if(person == null) {
                        terminal.print(prefix + "\nОчередь в ресторане кончилась. Скоро закрываемся");
                        return;
                    ...
                    terminal.print(prefix + "\nЯ не съел блюдо (попытка № " + person.getAttempt() + ")");
                }
